    ...
    <div>
      ...
      <ul className="space-y-1">
        <li>
          ...
          <strong>{formatWeight(gainPerWeek, useMetric)}/week</strong> of total weight.{" "}
        </li>
        <li className="mt-4">
        ...
        {distanceToGoal !== undefined &&
          (distanceToGoal === 0 ? (
            ...
            <>
              <li>
                You have <strong>{formatWeight(Math.abs(distanceToGoal), useMetric)}</strong> to{" "}
        ...
        {showCalories && ppw && ppw <= 0 && (
          <>
            <li className="mt-4">
              You are burning <strong>{formatNumber(caloriesPerDay)} cal/day</strong>{" "}
              {weightSlope > 0 ? "less" : "more"} than you are eat.
            </li>
            <li>
              {caloriesVsPlan < 0 ? (
                <>
                  You are burning <strong>{formatNumber(-1 * caloriesVsPlan)} cal/day</strong>{" "}
                  beyond your plan.
                </>
              ) : (
                <>
                  You must cut <strong>{formatNumber(caloriesVsPlan)} cal/day</strong> to lose{" "}
                  {formatWeight(-1 * ppw, useMetric)}/week.
                </>
              )}
            </li>
          </>
