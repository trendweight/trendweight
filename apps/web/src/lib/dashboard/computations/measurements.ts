...
export const computeMeasurements = (data?: SourceData[], profile?: ProfileData) => {
  ...
  let sourceMeasurements = Object.keys(groupedByDay)
    .map((key) => {
      ...
      return dayMeasurements.toSorted((a, b) => 
  ...
  sourceMeasurements = [...sourceMeasurements, ...missingDays]
    .toSorted((a, b) => a.date.toString().localeCompare(b.date.toString()))
  ...
  const measurementsByDate = Object.fromEntries(
    measurements.map(m => [m.date.toString(), m])
  )
  ...
  fatSourceMeasurements = [...fatSourceMeasurements, ...missingFatDays]
    .toSorted((a, b) => a.date.toString().localeCompare(b.date.toString()))
